  ...
  const events = [
    {
      ...
      description:
        "Join us for a groundbreaking dance-theater production that challenges norms and celebrates authenticity.",
    ...
    },
    {
      ...
      description:
        "Calling all professionals committed to equality! Join us to create a holistic support network.",
      month: "april",
      featured: true,
      price: "Free",
    ...
    },
    {
      ...
      description:
        "Join us for a vibrant celebration of Pride Month with music, art, and community activities.",
      month: "june",
      price: "Free Entry",
    ...
    },
    {
      ...
      description:
        "An interactive session focused on mental health awareness and support systems.",
      month: "may",
      price: "₹300",
    ...
    },
    {
      ...
      capacity: "30 spots",
      description:
        "Share your poetry, music, or stories in a supportive environment.",
      month: "april",
      price: "Free Entry",
    ...
    },
    {
      ...
      capacity: "40 spots",
      description:
        "Understanding gender identity and fostering inclusivity in our communities.",
      month: "april",
      price: "Free",
    ...
    },
    {
      ...
      capacity: "80 spots",
      description:
        "A curated selection of short films highlighting LGBTQ+ experiences.",
      month: "may",
      price: "₹200",
  ...
  const generateCalendarGrid = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      grid.push(
        <div
          key={`empty-${i}`}
          className="h-24 border border-neutral-100 bg-neutral-50/50"
        ></div>
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const isSelected =
        selectedDate && dateString === selectedDate.toISOString().split("T")[0];
      ...
      grid.push(
        <div
          key={day}
          className={`relative h-24 md:h-32 border border-neutral-100 p-1 transition-all duration-200 overflow-hidden ${isSelected
        ...
        >
          <div className="flex justify-between">
            <span
              className={`text-sm font-medium rounded-full w-6 h-6 flex items-center justify-center ${isToday ? "bg-primary-500 text-white" : ""
                }`}
            >
              {day}
            </span>
            ...
            {hasEvents && (
              <span className="flex space-x-1">
                {Array.from(new Set(dayEvents.map((e) => e.category))).map(
                  (category, i) => (
                    <span
                      key={i}
                      className={`w-2 h-2 rounded-full bg-${eventCategories.find((cat) => cat.id === category)
                          ?.color || "primary-500"
                        }`}
                    ></span>
                  )
          ...
          <div className="mt-1 space-y-1">
            {dayEvents.slice(0, 2).map((event) => (
              <div
                key={event.id}
                className={`text-xs truncate p-1 rounded bg-${eventCategories.find((cat) => cat.id === event.category)
                    ?.color || "primary-500"
                  }/10 text-${eventCategories.find((cat) => cat.id === event.category)
                    ?.color || "primary-500"
                  } border-l-2 border-${eventCategories.find((cat) => cat.id === event.category)
                    ?.color || "primary-500"
                  }`}
              >
      ...
      <section className="relative py-16 overflow-hidden bg-gradient-to-r from-primary-100 to-primary-50">
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-display font-bold text-neutral-800 mb-2">
                Event Calendar
              </h1>
              <p className="text-neutral-600">
                See all our upcoming events in a monthly view
              </p>
            </div>

            <Link
              to="/events"
              className="flex items-center px-4 py-2 bg-white rounded-xl border border-neutral-200 text-primary-600 hover:bg-primary-50 transition-all duration-200"
            >
              ...
              {showCategoryFilter && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-medium border border-neutral-100 p-2 z-50">
                  {eventCategories.map((category) => (
                    <button
                      key={category.id}
                      className={`w-full text-left px-3 py-2 rounded-lg text-sm mb-1 ${selectedCategory === category.id
                  ...
                  {event.image && (
                    <div className="h-40 relative">
                      ...
                      <div className="absolute top-3 left-3">
                        <span
                          className={`inline-block px-3 py-1 bg-white/90 backdrop-blur-sm text-${eventCategories.find(
