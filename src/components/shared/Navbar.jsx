  ...
  const isActivePath = (path) => {
    ...
    if (path === null && dropdownOpen) {
      const activeItem = navigationItems.find(
        (item) => item.name === dropdownOpen
      );
      if (activeItem && activeItem.submenu) {
        return activeItem.submenu.some(
                  ...
                  item.submenu ? (
                    ...
                    >
                      ...
                      >
                        ...
                        <ChevronDown
                          size={16}
                          className={`ml-1 transition-transform duration-200 ${dropdownOpen === item.name ? "rotate-180" : ""
                            }`}
                        />
                      </button>
                  ...
                  item.submenu ? (
                    <div key={index} className="space-y-2">
                      ...
                      >
                        ...
                        <ChevronDown
                          size={16}
                          className={`transition-transform duration-200 ${dropdownOpen === item.name ? "rotate-180" : ""
                            }`}
                        />
                      </button>
        ...
        {dropdownOpen === "mobile-programs" && (
          <div className="absolute bottom-16 left-0 right-0 bg-white border-t border-neutral-200 shadow-lg py-4 px-4 rounded-t-xl">
            ...
            <div className="grid grid-cols-2 gap-2">
              {navigationItems
                .find((item) => item.name === "Our Programs")
                ?.submenu.map((subItem, subIndex) =>
                  subItem.isExternal ? (
                    <a
                      ...
                      onClick={() => setDropdownOpen(null)}
                    >
                      {subItem.name === "Gazra Connect" && (
                        <BookOpen size={16} />
                      )}
                      {subItem.name === "Gazra Support Fund" && (
                        <Heart size={16} />
                      )}
                      {subItem.name === "Gazra Cafe" && <Coffee size={16} />}
                      {subItem.name === "Gazra Skill Hub" && (
                        <BookOpen size={16} />
                      )}
